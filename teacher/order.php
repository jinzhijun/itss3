...
include_once("../inc/new_header.php");
  ...
  $uid = $result_uid['0'];
  $sql = "SELECT count(*) FROM it_order_item WHERE teacher_id = $uid AND status = 3";
  $count = $db_pdo->query($sql);
  $count_1 = $count->fetchAll(PDO::FETCH_NUM);
  $total = $count_1[0][0];
  $num = 8;
  $page = new Page($total,$num);
  $sql_1 = "SELECT * FROM it_order_item WHERE teacher_id = $uid AND status = 3 {$page->limit}";
...
<table width="100%" border="0" cellspacing="20" cellpadding="0" class="wrap" id="teacher_box">
  <tr>
    ...
    <a href="order.php" class="on">已完成</a>
